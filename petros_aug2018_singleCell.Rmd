...
s1.data <- Read10X(data.dir = "C:/Users/rhooa/Documents/Projects/scRNA-Seq-10x/S1/filtered_gene_bc_matrices/GRCm38_ai9")
s1 <- CreateSeuratObject(raw.data = s1.data, project = "P21_Str")

s2.data <- Read10X(data.dir = "C:/Users/rhooa/Documents/Projects/scRNA-Seq-10x/S2/filtered_gene_bc_matrices/GRCm38_ai9")
s2 <- CreateSeuratObject(raw.data = s2.data, project = "P21_Ctx")

pbmc <- MergeSeurat(object1 = s1, object2 = s2, add.cell.id1 = "Str", 
    add.cell.id2 = "Ctx", project = "P21")
```

```{r}
# notice the cell names now have an added identifier
head(x = pbmc@cell.names)
...
pbmc <- FilterCells(object = pbmc, subset.names = c("nGene", "percent.mito"), low.thresholds = c(500, -Inf), high.thresholds = c(8000, 0.05))
```
***
...
pbmc <- SetAllIdent(object = pbmc, id = "orig.ident")
TSNEPlot(object = pbmc)
```

```{r tsneplot1, fig.height=5, fig.width=7}
# note that you can set do.label=T to help label individual clusters
pbmc <- SetAllIdent(object = pbmc, id = "res.0.6")
TSNEPlot(object = pbmc, do.label = TRUE, pt.size = 3)
```
...
```{r saveobject}
saveRDS(pbmc, file = 
          "C:/Users/rhooa/Documents/Projects/scRNA-Seq-10x/interneuron.rds")
```
...
pbmc.markers <- FindAllMarkers(object = pbmc, only.pos = FALSE, min.pct = 0.25, 
    thresh.use = 0.25)
...
genes_reporter = c("TOMA9")
genes_nsc = c("Gfap", "Nes", "Vim", "Pax6", "Sox2")
genes_astro = c("Slc1a3", "Slc1a2", "Glul", "S100b", "Aldh1l1")
genes_oligo = c("Cspg4", "Olig1", "Olig2", "Cldn11", "Mbp", "Sox10")
genes_microglia = c("Itgam", "Ptprc", "Cd40", "Aif1")
genes_npc = c("Eomes", "Ascl1", "Lhx6", "Lhx8", "Dcx")
...
genes_dopa = c("Th", "Pitx3", "Nr4a2", "Slc6a3", "Foxa2", "Kcnj6", "Lmx1b")

VlnPlot(object = pbmc, features.plot = genes_reporter, size.x.use = 8, x.lab.rot = TRUE)
VlnPlot(object = pbmc, features.plot = genes_nsc, size.x.use = 8, x.lab.rot = TRUE)
VlnPlot(object = pbmc, features.plot = genes_astro, size.x.use = 8, x.lab.rot = TRUE)
VlnPlot(object = pbmc, features.plot = genes_oligo, size.x.use = 8, x.lab.rot = TRUE)
VlnPlot(object = pbmc, features.plot = genes_microglia, size.x.use = 8, x.lab.rot = TRUE)
VlnPlot(object = pbmc, features.plot = genes_npc, size.x.use = 8, x.lab.rot = TRUE)
VlnPlot(object = pbmc, features.plot = genes_immature, size.x.use = 8, x.lab.rot = TRUE)
VlnPlot(object = pbmc, features.plot = genes_mature, size.x.use = 8, x.lab.rot = TRUE)
VlnPlot(object = pbmc, features.plot = genes_glut, size.x.use = 8, x.lab.rot = TRUE)
VlnPlot(object = pbmc, features.plot = genes_inter, size.x.use = 8, x.lab.rot = TRUE)
VlnPlot(object = pbmc, features.plot = genes_dopa, size.x.use = 8, x.lab.rot = TRUE)

# you can plot raw UMI counts as well
#VlnPlot(object = pbmc, features.plot = genes, use.raw = TRUE, y.log = TRUE)

```
```{r markerplots2, fig.height=8, fig.width=15,}
genes_subtypes = c("Pvalb", "Sst", "Calb2", "Chat", "Htr3a", "Vip", "Reln", "Nos1", "Npy", "Cck")
VlnPlot(object = pbmc, features.plot = genes_subtypes, size.x.use = 8, x.lab.rot = TRUE)
```


```{r}
cell_types = data.frame(cluster = c(0:10), type = c("PV/CCK", "oligo", "SST/Reelin", "PV", "SST/CR/Reelin/NPY", "SST/nNos/NPY (Ivy)", "5-HT3AR", "OPC", "SST/CCK", "astro", "ChaT"))
cell_types
```
```{r}
FeaturePlot(object = pbmc, features.plot = c(genes_reporter, "Slc1a3", "Mbp", "Rbfox3"), cols.use = c("grey", "blue"), reduction.use = "tsne")
FeaturePlot(object = pbmc, features.plot = c("Slc6a1", "Gad2", genes_subtypes), cols.use = c("grey", "red"), reduction.use = "tsne")
```

# ```{r}
# # pbmc <- SetAllIdent(object = pbmc, id = "orig.ident")
# # FeatureHeatmap(object = pbmc, features.plot = genes_subtypes, pt.size = 0.25, key.position = "top") 
# #    max.exp = 3)
# ```
...
new.cluster.ids <- cell_types$type
